...
*/
 ...
 /*       String TRENDING_ISSUE = "Trending Issue";
    ...
    public static String JsonStateList = "{\n" +
            "  \"state\":[\n" +
            "      {\n" +
    ...
    public static int getStateId(String stateName){
        ...
        try {
            jObject = new JSONObject(JsonStateList);
           JSONArray stateList = jObject.getJSONArray("state");
           for(int i = 0; i<stateList.length(); i++) {
               JSONObject jsonObject = stateList.getJSONObject(i);
               if (stateName.equals(jsonObject.get("key"))){
                   stateId = (int) jsonObject.get("value");
               }
           }
